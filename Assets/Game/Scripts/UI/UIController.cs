...
public class UIController : Singleton<UIController>
{
    ...
    private void ContinueInitialUI_1()
    {
        ...
        _ufo.DOScale(1.4f, _animationTime).SetDelay(_initialWaitTime).OnComplete(ContinueInitialUI_2);

        _animationTime *= 2;
    }
    private void ContinueInitialUI_2()
    {
        _planeta2.DOAnchorPos(_planeta2EndPos, _animationTime).SetDelay(_initialWaitTime / 2);
        _ufo.DOAnchorPos(_ufoEndPos, _animationTime).SetDelay(_initialWaitTime / 2).OnStart(GamePlayPanelUI);
    }
